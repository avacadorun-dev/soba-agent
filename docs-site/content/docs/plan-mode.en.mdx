...
ACP clients use `elicitation.form`. The terminal TUI uses an inline blocking dialog: the model turn pauses, while the
input remains responsive. Choose with `1`–`5`, arrows/Tab and Enter, click an option, type a free-form answer when
**Other** is enabled, press Esc to decline, or Ctrl+C to cancel. Clients without a structured clarification capability do
not receive the tool; SOBA keeps the question visible in the final response instead.

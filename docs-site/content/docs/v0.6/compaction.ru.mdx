...
SOBA создаёт **Context Capsules** — портативные сжатые представления контекста проекта, которые
...
При очень длинных сессиях может быть несколько капсул. SOBA объединяет их:
последняя капсула идёт в системный промпт полностью, более старые — сжимаются до
однострочных резюме.
...
### 3.2. Turn Complete (deferred preflight)
...
### 3.3. Milestone (deferred preflight)
...
| `timeoutMs` | `15000` | Таймаут preflight compact в мс |

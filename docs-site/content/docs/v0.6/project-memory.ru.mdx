...
description: "Постоянная память проекта в SOBA v0.6.x: knowledge files, memory capsules, entity graph, agent tools и безопасное обновление."
---
...
Capsule может хранить source receipt. В prompt просите поля явно: `source.file`, `source.lines`,
`source.lastVerified`, `source.confidence` и `staleIfFilesChange`.
...
`graph.json` хранит связи между сущностями проекта. В v0.6.x это локальный persisted graph слой для файлов, функций,
...
## 5. Project Memory doctor и Memory health commands
...
Memory receipt explanations читает `soba memory explain <query>`. Команда показывает релевантные capsules, source
receipts, score и doctor issues, чтобы было понятно, можно ли использовать найденный memory факт сейчас.

...
SOBA v0.6.0 работает как MCP-клиент: запускает локальные stdio MCP-серверы или подключается к remote Streamable HTTP
endpoints, читает `tools/list` и публикует готовые tools в общий `ToolRegistry`.

Не входят в scope v0.6.0: SOBA как MCP-сервер, marketplace discovery, signed servers, deprecated HTTP+SSE как отдельный
...
| Missing tools capability | Server не отдаёт MCP tools | В v0.6.0 нужны tools-capable MCP servers. |

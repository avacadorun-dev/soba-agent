...
| `timeoutMs` | `15000` | preflight compaction timeout |
...
当 effective tokens 超过 `hardLimit` 时，SOBA 会执行 blocking compaction。这个行为不能通过 `auto: false` 关闭。完整 compaction guide 当前在俄语文档中：[Compaction and Context Capsules](/ru/docs/v0.6/compaction)。

...
| `maxCompletionTokens` | `number` | `0` | transport 支持时的 output + reasoning token 上限；`0` 保留 provider 默认值 |
| `reasoning` | `ReasoningSelection` | `{ "mode": "provider_default" }` | 请求的 reasoning 策略；effective 值根据 active model capabilities 重新计算 |
| `contextWindow` | `number` | `128000` | 模型上下文窗口。优先使用 provider metadata；fallback 会在 UI 中标为 assumed |
...
| `theme` | `TuiThemeName` | `"graphite"` | TUI 主题：`graphite`、`vscode`、`github`、`aurora`、`synthwave`、`paper`、`forest`、`highContrast`、`clay`、`operator`、`ink` |
...
| `SOBA_REASONING_EFFORT` | `reasoning` | `default`、`none`、`minimal`、`low`、`medium`、`high`、`xhigh` 或 `max` |
| `SOBA_REASONING_BUDGET` | `reasoning` | 正整数 reasoning budget |
| `SOBA_REASONING_ENABLED` | `reasoning` | toggle 模型使用 `true`/`1` 或 `false`/`0` |
...
| `OPENAI_API_KEY` | OpenAI provider key | 官方 OpenAI API key |
...
| `--reasoning-budget <n>` | 设置正整数 reasoning budget |
| `--reasoning-enabled` | 为 toggle 模型启用 reasoning |
| `--no-reasoning` | 模型允许时关闭 reasoning |
...
SOBA v0.6.x 使用 provider registry。更多细节见 [Provider 与模型](/zh/docs/v0.6/providers)。
...
Discovery 也会读取 context/output limits 与 reasoning capabilities。限额优先级为：用户显式设置 → runtime → active route → model metadata → 精确内置 profile → 保守 fallback。SOBA 不会从模型名称猜测 capabilities；assumed 值会显示 `~`，选择时也会警告。
...
模型特定的 wire-format 兼容性应在完整的 provider JSON 中声明，并通过 `soba provider add <id> --from-file <path>` 加载。例如，`"compatibility": ["single_system_message"]` 会让 SOBA 为拒绝多条 system messages 的 chat template 只发送一条位于开头的 system message。完整 schema 和支持的 features 请参阅[模型兼容性元数据](/zh/docs/v0.6/providers#43-模型兼容性元数据)。
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `auto` | `true` | 开启软 deferred preflight compaction |
| `compactOnTurnComplete` | `true` | turn 完成后标记 deferred preflight compaction |
| `compactOnMilestone` | `true` | milestone checkpoint 时标记 pending compaction |
...
| `autoCompactThresholdRatio` | `0.8` | 相对于请求 hard limit 的软阈值 |
| `timeoutMs` | `15000` | 模型摘要超时后切换到 deterministic fallback |
| `backgroundTimeoutMs` | — | `timeoutMs` 的 deprecated alias |
...
当 effective tokens 超过 `hardLimit` 时，SOBA 会执行 blocking compaction。这个行为不能通过 `auto: false` 关闭。完整生命周期与阈值说明请参阅 [Compaction 与 Context Capsules](/zh/docs/v0.6/compaction)。
...
- **Reasoning 优先级：** request/session override → CLI → env → config → provider default。`/reasoning` 与 `F4` 无需重启即可影响下一次 model request。
- Requested 与 effective reasoning 分开保存；不支持的模式会明确 fallback 到 provider default，不会静默 clamp。
- **`maxOutputTokens` 和 `contextWindow`** 会从 active model 和 provider/runtime/route metadata 获取。CLI 与显式 model limits 可以覆盖 discovery。
- **`compaction.auto`** 可以通过 `SOBA_AUTO_COMPACT=false` 或 `--no-auto-compact` 覆盖。
- 关闭 auto-compaction 只会关闭软触发器；hard limit 和 provider overflow 恢复仍然强制执行。
- turn 和 milestone 只记录 pending intent；SOBA 会在下一次 model call 前显示进度并等待 compaction 完成。

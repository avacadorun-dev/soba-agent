...
支持的 compatibility features 是 `adaptive_thinking`、`reasoning_split`、`reasoning_details_input` 和 `prefer_max_completion_tokens`。`supportsThinking` 只是供界面使用的能力元数据，不会隐式启用任何 wire-format feature。动态 discovery 也会读取 upstream 的 `soba_compatibility` 或 `compatibility` 列表，并忽略未知值。

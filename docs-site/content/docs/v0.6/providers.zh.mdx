...
**v0.6.0 的重要变化：** 模型不再保存在硬编码列表里。SOBA 会通过 `GET /v1/models` 动态发现模型，所以 provider 新增模型后，通常不需要等待 SOBA 发版。
...
SOBA v0.6.0 提供 4 个内置 providers。它们都使用 OpenAI-compatible adapter。

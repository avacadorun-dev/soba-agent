...
| `maxCompletionTokens` | `number` | `0` | Максимум output + reasoning токенов там, где transport это поддерживает. `0` оставляет дефолт провайдера |
| `reasoning` | `ReasoningSelection` | `{ "mode": "provider_default" }` | Запрошенная reasoning-политика; effective-значение пересчитывается по capabilities активной модели |
| `contextWindow` | `number` | `128000` | Контекстное окно модели. При наличии используются метаданные провайдера; fallback явно помечается в UI как assumed |
...
| `theme` | `TuiThemeName` | `"graphite"` | Цветовая тема TUI. Доступные: `"graphite"`, `"vscode"`, `"github"`, `"aurora"`, `"synthwave"`, `"paper"`, `"forest"`, `"highContrast"`, `"clay"`, `"operator"`, `"ink"` |
...
| `SOBA_MAX_TOKENS` | `maxOutputTokens` | Устаревший alias для `SOBA_MAX_OUTPUT_TOKENS` |
| `SOBA_MAX_COMPLETION_TOKENS` | `maxCompletionTokens` | Лимит reasoning токенов |
| `SOBA_REASONING_EFFORT` | `reasoning` | `default`, `none`, `minimal`, `low`, `medium`, `high`, `xhigh` или `max` |
| `SOBA_REASONING_BUDGET` | `reasoning` | Положительный числовой reasoning budget |
| `SOBA_REASONING_ENABLED` | `reasoning` | `true`/`1` или `false`/`0` для toggle-моделей |
...
| `SOBA_CONFIG_PATH` | путь к config file | Использовать нестандартный файл конфигурации |
| `NO_COLOR` | output colors | Отключить ANSI-цвета, как `--no-color` |
...
| `OPENAI_API_KEY` | `apiKey` (для провайдера openai-official) | API-ключ OpenAI |
...
| `--reasoning-budget <n>` | Задать положительный числовой reasoning budget |
| `--reasoning-enabled` | Включить reasoning для toggle-модели |
| `--no-reasoning` | Выключить reasoning, если модель это разрешает |
...
SOBA v0.6.x использует систему реестра провайдеров (registry). Подробнее о провайдерах — в
[гайде по провайдерам](/ru/docs/v0.6/providers).

### 5.1. Встроенные провайдеры

SOBA поставляется с 5 встроенными провайдерами:
...
Discovery также читает context/output limits и reasoning capabilities. Приоритет лимитов: явная настройка пользователя → runtime → active route → model metadata → точный встроенный профиль → консервативный fallback. SOBA не угадывает capabilities по имени модели; assumed-значения отмечаются `~` и предупреждением при выборе.
...
Особенности wire-формата конкретной модели задаются в полном JSON-описании провайдера, загружаемом через `soba provider add <id> --from-file <path>`. Например, `"compatibility": ["single_system_message"]` заставляет SOBA отправлять одно начальное системное сообщение для chat templates, которые отклоняют несколько таких сообщений. Полная схема и список features приведены в разделе [метаданных совместимости модели](/ru/docs/v0.6/providers#43-метаданные-совместимости-модели).
...
|---|---|---|
| `auto` | `true` | Включить мягкий deferred preflight compaction |
| `compactOnTurnComplete` | `true` | Пометить compaction как pending после завершения хода |
| `compactOnMilestone` | `true` | Пометить compaction как pending при milestone checkpoint |
...
| `autoCompactThresholdRatio` | `0.8` | Мягкий порог как доля hard limit запроса |
| `timeoutMs` | `15000` | Бюджет модельного summary до deterministic fallback, в мс |
| `backgroundTimeoutMs` | — | Устаревший alias для `timeoutMs` |
...
- **Reasoning:** request/session override → CLI → env → config → provider default. `/reasoning` и `F4` меняют следующий model request без перезапуска.
- Requested и effective reasoning хранятся отдельно. Неподдерживаемый режим даёт видимый fallback к provider default без скрытого clamp.
- **`maxOutputTokens` и `contextWindow`**: извлекаются из активной модели и метаданных provider/runtime/route.
  CLI-флаг `--max-output-tokens`, `--context-window` или явные model limits переопределяют discovery для локальных прокси и неполных каталогов.

- **`auto` в compaction**: `SOBA_AUTO_COMPACT=false` или `--no-auto-compact` переопределяет `compaction.auto`.
- Отключение auto-compaction выключает только мягкие trigger. Hard limit и восстановление после provider overflow остаются обязательными.
- Trigger после turn и milestone откладывается: SOBA показывает прогресс и ожидает compact непосредственно перед следующим model call.

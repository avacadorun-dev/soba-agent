...
Встроенный список правил поддерживается runtime trust policy.
...
Запрос также может показывать более безопасные alternatives. SOBA сейчас предлагает least-privilege rewrites для
частых risky-паттернов: destructive cleanup, связанный с build-командой, и remote `git push`. Эти alternatives
записываются в proof permission receipt, чтобы потом `soba prove` и `soba verify` показывали, что именно было
...
Conversation items в session JSONL сохраняют user/model content для resume, поэтому этот файл следует считать
чувствительными локальными данными. Flight/debug sidecars редактируют secret-bearing keys и распознаваемые credentials
в persisted prompt/output strings, но credentials всё равно нельзя помещать в сессию.

Proof receipts добавляют отдельный локальный audit artifact в `.soba/evidence/`. Новые receipts рекурсивно редактируются,
sealed SHA-256 digest и на POSIX записываются с правами только владельца. `soba verify` обнаруживает изменение файла и
отклоняет successful evidence, связанный с denied tool call. Это не превращает workspace в sandbox и не заменяет
аккуратную работу с credentials: redaction не умеет распознавать любой возможный формат секрета.

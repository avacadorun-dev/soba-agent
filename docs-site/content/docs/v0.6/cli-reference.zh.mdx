...
Permission receipts 会记录每个已执行 tool call 的 trust level、approval kind、approval value、decision 和 reason；`auto` 表示 SOBA trust policy 无需提示即可允许该调用。
`soba verify` 会校验 proof schema，并检查 supported claim 引用是否指向已知的 evidence、command、check、file mutation 或 risk ID。

| 命令 | 作用 |
|---|---|
| `soba prove` | 显示当前项目的最新 proof bundle |

...
当前 proof bundles 包含紧凑 evidence index、finish claims、changed files、checks、commands、risks 和 review actions。
...
`soba prove` 和 `soba verify` 不启动 agent runtime，也不需要 provider credentials。

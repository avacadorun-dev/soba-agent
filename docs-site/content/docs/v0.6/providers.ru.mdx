...
description: Встроенные провайдеры DeepSeek, Moonshot Kimi, Alibaba Qwen, OpenRouter и OpenAI. Динамический discovery моделей и кастомные провайдеры.
---
...
- **Встроенные провайдеры**: 5 провайдеров с известными base URL и transport-профилями
...
SOBA v0.6.x поставляется с 5 встроенными провайдерами. Официальный OpenAI provider использует нативный Responses API, остальные — специализированные OpenAI-compatible transports.
...
| `openai-official` | OpenAI | `https://api.openai.com/v1` | `OPENAI_API_KEY` | Нативный Responses API, включая opaque reasoning items |
...
Discovery также читает лимиты контекста/вывода и структурированные reasoning capabilities, если provider их публикует. Приоритет effective limits: явные значения модели пользователя → runtime limit → лимит активного route → общий лимит модели → поддерживаемый профиль точной модели → консервативный fallback. Лимит router route может сузить более широкое context window модели. Fallback-значения помечаются как предполагаемые (`~`), а при выборе такой модели SOBA показывает предупреждение.
...
### 3.3. Управление reasoning

SOBA хранит provider-neutral запрошенную policy и вычисляет effective policy для активной модели. Нажмите `F4`, чтобы циклически переключать только те controls, которые реально объявляет модель, или используйте `/reasoning` для точного effort, toggle либо token budget. В sidebar показывается effective mode; при несовместимом выборе он отображает `effective ← requested` и сообщает причину fallback.

`provider default` означает, что reasoning-поле вообще не отправляется. SOBA не заменяет неподдерживаемый уровень ближайшим. Если live capabilities устарели и provider отклонил reasoning-параметр, запрос один раз повторяется без reasoning-полей, а fallback фиксируется.

### 3.4. Рекомендации по моделям
...
| `--model <spec>` | Да | Модель в формате `id[=name][,contextWindow[,maxOutput[,supportsStreaming[,supportsThinking]]]]`. Лимиты можно не указывать и получить через discovery. Можно повторять |
| `--default-model <id>` | Нет | Default model. Если не указан, используется первый `--model` |
| `--adapter <openai\|openai-responses\|anthropic>` | Нет | Адаптер провайдера. `openai-responses` используйте только для endpoint с Responses API |
| `--metadata-profile <profile>` | Нет | Discovery-профиль: `auto`, `generic_openai`, `openrouter`, `vllm`, `ollama`, `lmstudio`, `llamacpp` или `none` |
...
Поддерживаются `adaptive_thinking`, `reasoning_split`, `reasoning_details_input`, `prefer_max_completion_tokens` и `single_system_message`. Legacy boolean `supportsThinking` читается ещё один compatibility-релиз, но не описывает уровни и бюджеты; для новых definitions используйте структурированное поле `reasoning`. Dynamic discovery принимает тот же список из upstream-полей `soba_compatibility` или `compatibility` и игнорирует неизвестные значения.

Reasoning `control` может быть `none`, `effort`, `budget`, `toggle` или `fixed`. Transport тоже задаётся явно: `openai_chat`, `openai_responses`, `openrouter`, `deepseek`, `kimi`, `minimax`, `qwen`, `ollama` или `none`. Capability declaration определяет доступный пользователю выбор, transport — его сериализацию. Неизвестный endpoint не получает reasoning-поля без подтверждённого профиля.

`metadataProfile` управляет только разбором model catalogue metadata. `auto` подходит большинству custom endpoints и использует endpoint probes для поддержанных локальных серверов; `vllm`, `ollama`, `lmstudio` и `llamacpp` выбирают их документированные metadata shapes. `none` отключает интерпретацию provider metadata. Это не связано с reasoning transport.

Используйте `single_system_message` для chat templates, которые принимают не более одного системного сообщения. На границе OpenAI wire-формата SOBA объединяет основные инструкции, инструкции активных skills, context capsules и compaction summaries в одно системное сообщение с индексом 0. Относительный порядок несистемных сообщений сохраняется. Флаг задаётся для каждой модели отдельно, поэтому модели одного endpoint могут иметь разные настройки совместимости.

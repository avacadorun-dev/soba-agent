...
| `--reasoning-effort <level>` | Request `default`, `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max` |
| `--reasoning-budget <n>` | Request a positive numeric reasoning budget |
| `--reasoning-enabled` | Enable a toggle-based reasoning control |
| `--no-reasoning` | Disable reasoning when supported |
...
| `--adapter <openai\|openai-responses\|anthropic>` | Adapter id. Use `openai-responses` only for an endpoint that implements the Responses API |
| `--metadata-profile <profile>` | Discovery profile: `auto`, `generic_openai`, `openrouter`, `vllm`, `ollama`, `lmstudio`, `llamacpp`, or `none` |
...
Limits are optional. For example, `--model local-code="Local Code" --metadata-profile vllm` lets SOBA fill context/output values from the endpoint when it exposes them. If metadata is absent, SOBA uses conservative assumed values and marks them with `~`; pass explicit numbers when the server catalogue is incomplete.

Structured reasoning capabilities and the provider-specific wire transport are declared in a full JSON definition loaded with `--from-file`. SOBA never sends reasoning fields to an unknown endpoint merely because the model name looks familiar.

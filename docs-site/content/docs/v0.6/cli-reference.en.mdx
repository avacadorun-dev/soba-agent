...
Current proof bundles include a compact evidence index, finish claims, changed files, checks, commands, risks, and review actions.
`soba verify` validates the proof schema and checks that supported claim references point at known evidence, command, check, file mutation, or risk IDs.

...
| Command | Purpose |
|---|---|
| `soba prove` | Show the latest Verified Handoff for the current project |
| `soba prove --last` | Explicitly show the latest Verified Handoff |
| `soba prove --format markdown` | Render PR-ready Observed, Declared, Unknown, and Integrity sections |
| `soba prove --format json` | Print raw Proof Bundle v1 fields plus `proofPath` |
| `soba prove <path>` | Show a Verified Handoff for a specific `.soba-proof.json` file |

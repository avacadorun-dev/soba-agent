...
Permission receipts record each executed tool call's trust level, approval kind, approval value, decision, and reason; `auto` means SOBA's trust policy allowed the call without prompting.

...
| `F4` / `/reasoning` | 循环 active model 声明支持的 reasoning 模式 | 主区域 |
...
`F4` 只循环 active model 声明的 controls。effort 模型循环 provider default 和已公布的 levels；toggle 模型循环 provider default、on、off。numeric budget 必须先通过 `/reasoning budget <tokens>` 显式设置，因此快捷键不会凭空选择昂贵预算。

侧栏会在模型旁持续显示 Reasoning，切换后会短暂高亮。Fallback 以 `effective ← requested` 显示，例如 `provider default ← max`，command output 会说明原因。设置无需重启，直接作用于下一次 model request。
...
| 命令 | 动作 |
|---|---|
| `/reasoning` | 显示 requested/effective 模式与 fallback 原因 |
| `/reasoning default` | 不发送 reasoning 参数，使用 provider default |
| `/reasoning none\|minimal\|low\|medium\|high\|xhigh\|max` | active model 支持时请求 effort |
| `/reasoning on\|off` | 控制 toggle 模型 |
| `/reasoning budget <tokens>` | 设置显式 numeric thinking budget |
...
| `/skill eval <name>` | 为 draft skill 运行 deterministic eval cases |
| `/skill bench <name>` | 显示 eval 聚合表现和 common failures |
| `/skill trace <name>` | 显示最近的 eval runs 和 failed cases |
...
- **Trust 仍然生效**：`rm -rf`、`sudo` 等危险命令在 `safe` trust mode 下会被阻止，在 `normal` 下需要确认。见 [安全](/zh/docs/v0.6/security)。
...
每个主题都定义语义 token，例如 accent、text、muted、success、error、border，并支持深色和浅色模式。更多细节见[主题指南](/zh/docs/v0.6/themes)。
...
翻译文件位于 `locales/`。添加语言时，创建对应 JSON 文件，并在 `src/shared/i18n/locale-catalog.ts` 中添加一个条目；
locale 类型、验证、配置加载和命令用法都会从该 catalog 派生。

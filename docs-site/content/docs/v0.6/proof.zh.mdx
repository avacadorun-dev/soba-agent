...
当 completion gate 接受未显式提供 evidence IDs 的 criteria 时，SOBA 只会把它们关联到该 turn 中已经记录且
成功的 mutation、verification 和 inspection evidence。SOBA 不会虚构证据；没有匹配记录时，claim 仍为 unverified。

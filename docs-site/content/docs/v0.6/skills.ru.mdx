...
После eval SOBA сохраняет runs в локальном evaluation storage. Их можно просматривать как skill card:

```bash
# Агрегированное качество skill
/skill bench my-skill

# Последние eval runs и failed cases
/skill trace my-skill
/skill trace my-skill --last 5
```

`bench` показывает количество runs, success rate, median pass rate, median cases, latest run и common failures. `trace`
показывает последние runs с pass rate и причинами failed cases. Эти команды не запускают модель: они читают уже
сохранённые eval artifacts.

Skill eval bench and trace — обязательный цикл для скила, который должен использоваться повторно:

1. `/skill eval <name>` запускает deterministic evaluation cases и сохраняет локальные artifacts.
2. `/skill bench <name>` показывает агрегированное качество и common failures.
3. `/skill trace <name>` раскрывает последние eval runs и failed cases, чтобы следующая правка была точечной.

Не продвигайте project skill в user scope, пока eval, bench и trace не показывают согласованную картину.
...
| `/skill trace <name> [--last n]` | Показать последние eval runs и failed cases |
...
4. **Используйте eval/bench/trace** — `/skill eval <name>`, `/skill bench <name>` и `/skill trace <name>` для анализа

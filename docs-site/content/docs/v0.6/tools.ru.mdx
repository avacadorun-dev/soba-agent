...
Для dangerous-команд SOBA также записывает permission receipts. Когда можно вывести более безопасный путь, confirmation
prompt и proof receipt показывают least-privilege alternatives, например split `rm -rf dist && bun run build` на
scoped repo cleanup и отдельную недеструктивную build-команду.
...
Для долгоживущих фактов просите SOBA прикреплять source receipt, если источник можно проверить:

```text
Обнови Project Memory правилом загрузки provider registry.
Добавь source.file, source.lines, source.lastVerified, source.confidence и staleIfFilesChange.
```

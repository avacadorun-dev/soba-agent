...
对于 dangerous 命令，SOBA 还会记录 permission receipts。当 SOBA 能推断出更安全路径时，确认提示和 proof
receipt 会包含 least-privilege alternatives，例如把 `rm -rf dist && bun run build` 拆成 scoped repo cleanup
和单独的非破坏性 build 命令。

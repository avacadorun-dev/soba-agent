...
内置规则列表由 runtime trust policy 维护。
...
确认提示也可以显示更安全的 alternatives。SOBA 当前会为常见风险模式建议 least-privilege rewrites，包括与 build
命令串联的 destructive cleanup，以及远程 `git push`。这些 alternatives 会写入 proof permission receipt，
这样之后 `soba prove` 和 `soba verify` 可以显示实际批准或拒绝的内容。
...
Session JSONL 中的 conversation items 会保留 user/model content 以支持 resume，因此应把该文件视为敏感的本地数据。
Flight/debug sidecars 会脱敏 secret-bearing keys，以及持久化 prompt/output 字符串中可识别的 credentials，
但仍不应把 credentials 粘贴到会话中。

Proof receipts 会在 `.soba/evidence/` 中增加独立的本地 audit artifact。新 receipts 会递归脱敏、使用 SHA-256
digest sealing，并在 POSIX 系统上以仅所有者可读写的权限保存。`soba verify` 可以检测篡改，并拒绝与 denied
tool call 关联的 successful evidence。这不会把 workspace 变成 sandbox，也不能代替谨慎的 credential 处理；
redaction 无法识别所有可能的 secret 格式。

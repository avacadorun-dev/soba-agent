...
description: "Постоянная память проекта в SOBA v0.4.2: knowledge files, memory capsules, entity graph, agent tools и безопасное обновление."
---
...
В v0.4.2 Project Memory — это не slash-команда пользователя. Это слой памяти, который SOBA автоматически подключает к
...
`graph.json` хранит связи между сущностями проекта. В v0.4.2 это локальный persisted graph слой для файлов, функций,

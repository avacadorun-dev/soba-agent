...
模型特定的 wire-format 兼容性应在完整的 provider JSON 中声明，并通过 `soba provider add <id> --from-file <path>` 加载。例如，`"compatibility": ["single_system_message"]` 会让 SOBA 为拒绝多条 system messages 的 chat template 只发送一条位于开头的 system message。完整 schema 和支持的 features 请参阅[模型兼容性元数据](/zh/docs/providers#43-模型兼容性元数据)。
...
当 effective tokens 超过 `hardLimit` 时，SOBA 会执行 blocking compaction。这个行为不能通过 `auto: false` 关闭。完整生命周期与阈值说明请参阅 [Compaction 与 Context Capsules](/zh/docs/compaction)。

...
1. [环境变量](#3-环境变量)
1. [CLI 参数](#4-cli-参数)
1. [Provider 配置](#5-provider-配置)
1. [Compaction 配置](#6-compaction-配置)
1. [优先级](#7-优先级)
1. [校验](#8-校验)
...
| `model` | `string` | `""` | 完整模型 ID，例如 `deepseek/deepseek-chat` 或 `openrouter/anthropic/claude-sonnet-4` |
...
SOBA v0.4.0 使用 provider registry。更多细节见 [Provider 与模型](/zh/docs/providers)。

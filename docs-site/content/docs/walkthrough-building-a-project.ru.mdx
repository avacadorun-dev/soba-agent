...
description: "End-to-end руководство по SOBA Agent v0.4.4: TUI, безопасная разработка, Project Memory, MCP, skills, sessions, capsules, tests и handoff."
---

Это руководство показывает полный рабочий цикл разработки с SOBA Agent v0.4.4. Мы не будем делать игрушечный
...
## Что из v0.4.4 покрывает tutorial
...
Не входит в v0.4.4 scope: SOBA как MCP-сервер, marketplace discovery, team memory sync и автоматический memory-doctor.
...
Project Memory в v0.4.4 — это проектная память на диске, а не отдельная slash-команда. SOBA создаёт и читает:
...
MCP в v0.4.4 поддерживает local `stdio` servers и remote `streamableHttp` endpoints. В tutorial используется локальный
...
- Hot reload MCP config в v0.4.4 не поддержан: после правки `.soba/mcp.json` перезапустите SOBA.
...
Финальная проверка v0.4.4 workflow — новая сессия должна понимать проект без полного повторения контекста.

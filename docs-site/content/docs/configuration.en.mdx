...
1. [CLI flags](#4-cli-flags)
1. [Provider configuration](#5-provider-configuration)
1. [Compaction configuration](#6-compaction-configuration)
1. [Precedence](#7-precedence)
1. [Validation](#8-validation)
...
| `model` | `string` | `""` | Full model identifier, for example `deepseek/deepseek-chat` or `openrouter/anthropic/claude-sonnet-4` |
...
SOBA v0.4.0 uses a provider registry. See [Providers and models](/en/docs/providers) for details.
...
When effective tokens exceed `hardLimit`, SOBA runs blocking compaction. This cannot be disabled with `auto: false`. The full compaction guide is currently available in Russian: [Compaction and Context Capsules](/ru/docs/compaction).

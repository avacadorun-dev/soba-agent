...
SOBA v0.6.x uses a provider registry. See [Providers and models](/en/docs/providers) for details.
...
When effective tokens exceed `hardLimit`, SOBA runs blocking compaction. This cannot be disabled with `auto: false`. See [Compaction and Context Capsules](/en/docs/compaction) for the complete lifecycle and threshold guide.

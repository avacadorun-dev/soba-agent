...
SOBA v0.4.4 работает как MCP-клиент: запускает локальные stdio MCP-серверы или подключается к remote Streamable HTTP
endpoints, читает `tools/list` и публикует готовые tools в общий `ToolRegistry`.

Не входят в scope v0.4.4: SOBA как MCP-сервер, marketplace discovery, signed servers, deprecated HTTP+SSE как отдельный
...
| Missing tools capability | Server не отдаёт MCP tools | В v0.4.4 нужны tools-capable MCP servers. |

...
- **Trust still applies**: dangerous commands such as `rm -rf` or `sudo` are blocked in `safe` trust mode and require confirmation in `normal` mode. See [Security](/en/docs/security).
...
Each theme defines semantic tokens such as accent, text, muted, success, error, and border, with support for dark and light modes. See the [theme guide](/en/docs/themes) for details.

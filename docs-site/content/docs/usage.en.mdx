...
| `Ctrl+M` | Open the provider/model selector | Main area |

### Model selector

`Ctrl+M` opens a dense two-column selector. Providers are on the left, models for the highlighted provider are on the right, and the search box filters both provider and model names/ids.

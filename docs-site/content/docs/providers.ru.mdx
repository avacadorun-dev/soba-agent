...
Поддерживаются `adaptive_thinking`, `reasoning_split`, `reasoning_details_input`, `prefer_max_completion_tokens` и `single_system_message`. Поле `supportsThinking` — метаданные возможности для интерфейса; оно само не включает особенности wire-формата. Dynamic discovery принимает тот же список из upstream-полей `soba_compatibility` или `compatibility` и игнорирует неизвестные значения.

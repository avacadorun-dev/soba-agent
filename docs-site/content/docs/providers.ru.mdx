...
В TUI модель переключается через селектор `Ctrl+M`. Селектор показывает провайдеры в левой колонке, а модели выбранного провайдера — в правой. Поиск фильтрует и провайдеры, и названия/ID моделей; в строках моделей показываются context window, max output, streaming и thinking бейджи, если эти данные известны.

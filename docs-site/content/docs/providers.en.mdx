...
**Important in v0.4.4:** models are no longer kept in hard-coded lists. SOBA discovers them dynamically through `GET /v1/models`, so new provider models can appear without a SOBA release.
...
SOBA v0.4.4 ships with four built-in providers. All of them use the OpenAI-compatible adapter.

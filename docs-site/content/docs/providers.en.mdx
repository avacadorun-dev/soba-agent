...
**Important in v0.4.2:** models are no longer kept in hard-coded lists. SOBA discovers them dynamically through `GET /v1/models`, so new provider models can appear without a SOBA release.
...
SOBA v0.4.2 ships with four built-in providers. All of them use the OpenAI-compatible adapter.
...
In the TUI, switch models with `Ctrl+M`. The selector shows providers in the left column and models for the highlighted provider in the right column. Search filters both provider and model names/ids; each model row includes context window, max output, streaming, and thinking badges when known.

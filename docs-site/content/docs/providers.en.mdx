...
Supported compatibility features are `adaptive_thinking`, `reasoning_split`, `reasoning_details_input`, `prefer_max_completion_tokens`, and `single_system_message`. `supportsThinking` is display/capability metadata; it does not implicitly enable any wire-format feature. Dynamic discovery accepts the same list from upstream `soba_compatibility` or `compatibility` metadata and ignores unknown values.

...
**Important in v0.6.0:** models are no longer kept in hard-coded lists. SOBA discovers them dynamically through `GET /v1/models`, so new provider models can appear without a SOBA release.
...
SOBA v0.6.0 ships with four built-in providers. All of them use the OpenAI-compatible adapter.

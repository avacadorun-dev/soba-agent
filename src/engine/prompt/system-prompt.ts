...
const TOOL_SNIPPETS: Record<string, string> = {
  ...
  bash: "Run project commands, verification workflows, git, package-manager scripts, and shell-only operations. Prefer bounded file tools for pwd, ls/find/grep/rg/sed/cat inspection or routine reads when they fit.",
...
const CORE_GUIDELINES = [
  ...
  "Run final verification commands directly and let the tool truncate long output. Verification commands piped through head/tail do not count as passing evidence. Do not present --help, --version, which, command -v, type, or man probes as passed checks",

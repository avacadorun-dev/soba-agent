...
} from "../compaction/trigger-policy";
import type { I18n } from "../i18n/i18n";
import { discoverModels, toModelDefinitions } from "../provider/discovery";
import {
  DEFAULT_SYNTHETIC_CONTEXT_WINDOW,
  DEFAULT_SYNTHETIC_MAX_OUTPUT,
} from "../provider/model-defaults";
import { BUILTIN_PROVIDERS } from "../provider/providers";
import { ProviderRegistry } from "../provider/registry";
import type { ModelDefinition, ProviderDefinition } from "../provider/types";
...
async function readConfigFile(
  ...
  try {
    ...
    if (parsed.sound && typeof parsed.sound === "object" && !Array.isArray(parsed.sound)) {
      config.sound = { ...config.sound, ...parsed.sound } as Partial<
...
export function loadConfigFromEnv(): Partial<SobaConfig> {
  ...
  if (Object.keys(soundEnv).length > 0) {
    overrides.sound = { ...overrides.sound, ...soundEnv } as Partial<import("./types").SoundConfig>;
  }
